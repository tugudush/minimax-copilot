...
export class MiniMaxChatProvider implements vscode.LanguageModelChatProvider {
  ...
  constructor(private readonly context: vscode.ExtensionContext) {
    context.subscriptions.push(
      ...
      vscode.workspace.onDidChangeConfiguration((e) => {
        if (e.affectsConfiguration('minimax.visibleModels') || e.affectsConfiguration('minimax.apiBaseUrl')) {
          this._onDidChange.fire();
        }
      }),
    );
  ...
  async provideLanguageModelChatResponse(
    ...
    if (!info) {
      progress.report(new vscode.LanguageModelTextPart(`Unknown model: ${model.id}`));
      return;
    }
    ...
    // Stream.
    await streamChat(apiKey, model.id, system, anthropicMessages, maxTokens, progress, token);
  }
